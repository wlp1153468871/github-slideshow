  ...
  <div class="pvc-table">
    <div class="table-toolbar">
      <div class="table-toolbar-right">
        <div>
          <dao-input v-model="filterKey" search placeholder="请输入PVC的名称搜索"> </dao-input>
        </div>
      </div>
    </div>

    <el-table v-loading="loading" :data="pvcsInCurrentPage" style="width: 100%;">
...
export default {
  ...
  computed: {
    ...
    pvcFilteredByKey() {
      ...
      return this.pvcs.filter(volume => volume.metadata.name.toLowerCase().includes(filterKey));
    },
