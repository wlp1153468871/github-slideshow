<template>
  <div class="operating-data">
    ...
    <div class="operating-data-view" v-loading="loading">
        ...
        <table>
          <tbody>
            <tr
              ...
              :key="index"
            >
              <td class="operating-data-line-number" :data-line-number="index + 1"></td>
...
export default {
  ...
  methods: {
    ...
    loadTerminalHistories: debounce(function loadTerminalHistories(currentpage, goto) {
      ...
      TerminalHistoryService.ListOperatingData(this.space.id, this.name, query)
        .then(TerminalHistories => {
          ...
          if (logs === null) {
            ...
            this.histories = logs.map(
              item =>
                `dateTime:${item._source.dateTime}  podName:${item._source.podName}  user:${item._source.user}  message: ${item._source.message}`, // eslint-disable-line no-underscore-dangle
            );
          }
