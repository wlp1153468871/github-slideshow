<template>
  <div>
    <empty-state title="加载中" v-if="loading"> </empty-state>
    <empty-state title="容器组为空，无实时日志信息" v-else-if="!pods.length"> </empty-state>
    <pod-log :pod="pod" :pod-name="podName" v-else-if="pods.length">
      <label>Pod: </label>
...
export default {
  ...
  methods: {
    getPodService() {
      switch (this.type) {
        case 'deployment':
          return DeploymentResourceService.getPods(this.space.id, this.zone.id, this.name);
        case 'deploymentConfig':
          return DCService.getPodList(this.space.id, this.zone.id, this.name);
        case 'statefulSet':
          return StatefulSetService.getPodList(this.space.id, this.zone.id, this.name);
        default:
          return DeploymentResourceService.getPods(this.space.id, this.zone.id, this.name);
      }
    },
