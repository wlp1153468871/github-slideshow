    ...
    <table class="dao-table row">
      ...
      <tbody v-else>
        <template v-for="(ports, routeName) in portsByRoute">
          <tr v-for="(port, index) in ports" v-if="routeName !== ''" :key="routeName + index">
            <td>
              <resource-link kind="Route" :name="routeName"> </resource-link>
...
export default {
  ...
  computed: {
    noPorts() {
      return hashSizeFilter(this.portsByRoute) === 0 && this.unusedPorts.length === 0;
    },
  },
      ...
      each(this.service.spec.ports, port => {
        ...
        each(this.routesForService, route => {
          if (
            ...
            this.portsByRoute[route.metadata.name] = this.portsByRoute[route.metadata.name] || [];
