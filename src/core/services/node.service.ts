...
class NodeService {
  ...
  updateNode(name: string, node: any, zoneId: string) {
    return this.api.put(`/nodes/${name}`, node, { params: { zone: zoneId } });
  }

  getNodeDetail(name: string, zoneId: string) {
    return this.api.get(`/nodes/${name}`, { zone: zoneId });
  }
  ...
  getPodsDetail(namespace: string, podName: string,zoneId: string) {
    return this.api.get(`/namespaces/${namespace}/pods/${podName}`, { zone: zoneId });
  }

  deletePods(namespace: string, podName: string,zoneId: string) {
    return this.api.delete(`/namespaces/${namespace}/pods/${podName}`, { zone: zoneId });
  }

  getPodsEvents(namespace: string, podName: string,zoneId: string) {
    return this.api.get(`/namespaces/${namespace}/pods/${podName}/events`, { zone: zoneId });
  }

  ListTerminalHistories(namespace: string, podName: string, containerName: string,zoneId: string, query: object,) {
    return this.api.get(`/namespaces/${namespace}/pods/${podName}/containers/${containerName}/cmd`, { zone: zoneId, query });
  }

  executePodsCmd(namespace: string, podName: string, containerName: string,zoneId: string){
    return this.api.post(`/namespaces/${namespace}/pods/${podName}/containers/${containerName}/cmd`, { zone: zoneId });
  }

  getPodShell(namespace: string, podName: string, containerName: string,zoneId: string) {
    return this.api.get(`/namespaces/${namespace}/pods/${podName}/containers/${containerName}/shell`, { zone: zoneId });
  }
  ...
  listConfigMap(namespace: string,zoneId: string) {
    return this.api.get(`/namespaces/${namespace}/configmaps`, { zone: zoneId })
  }

  fetchContainerMonitorUrl(namespace: string, podName: string, instance: string, numberEx: string, from: string, to: string, zoneId: string, refresh: string,) {
    return this.api.get(`/namespaces/${namespace}/monitoring/instance/${instance}/number/${numberEx}`, {zone: zoneId, pod: podName, from ,to, refresh})
  }

  downloadLog(namespace: string, podName: string, containerName: string, zoneId: string, query: string) {
   return this.api.get(`/namespaces/${namespace}/pods/${podName}/log/download`, {zone: zoneId, container_name: containerName, query, responseType: 'blob' })
  }

  saveFile(namespace: string, podName: string, containerName: string,zoneId: string, path: string){
    return this.api.get(`/namespaces/${namespace}/pods/${podName}/download`, {zone: zoneId, containerName: containerName, path})
  }
}
